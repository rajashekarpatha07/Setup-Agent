...
def history(limit, json_out):
    ...
    table = Table(
        title="📁 Project History", box=box.ROUNDED,
        border_style="bright_blue", show_lines=False, padding=(0, 1),
    )
    ...
    for p in projects:
        ...
        table.add_row(str(p.get("id", "")), p.get("name", "unknown"), p.get("project_type", "-"), status, duration, date)
...
def templates():
    ...
    table = Table(title="📋 Available Templates", box=box.ROUNDED, border_style="bright_blue")
...
def check(project_name):
    ...
    if status == "healthy":
        console.print(Panel(f"[bold green]✅ HEALTHY[/] — {result['passed']}/{result['total']} checks passed", border_style="green"))
    else:
        console.print(Panel(f"[bold red]⚠️ UNHEALTHY[/] — {result['passed']}/{result['total']} checks passed", border_style="red"))
