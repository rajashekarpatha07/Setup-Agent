...
def _check_dir_exists(project_path: Path, dirname: str) -> dict:
    ...
    return {
        "check": f"{dirname}/ exists",
        "passed": True,
        "detail": f"Found with {len(items)} items",
    }
...
def _detect_project_type(project_path: Path) -> str:
    ...
    if (project_path / "package.json").exists():
        # Distinguish Node.js project types
        try:
...
def _check_nodejs(project_path: Path, project_type: str) -> list[dict]:
    ...
    if (project_path / "tsconfig.json").exists():
        ...
        checks.append(_run_check_command(
            project_path, "npx tsc --noEmit 2>&1",
    ...
    try:
        ...
        checks.append({
            ...
            "passed": True,
            "detail": f"Valid JSON with {dep_count} dependencies",
        })
    except Exception as e:
        checks.append({
            "check": "package.json is valid",
...
def _check_python(project_path: Path, project_type: str) -> list[dict]:
    ...
    checks = [
        _check_file_exists(project_path, "pyproject.toml"),
    ]

    # Check if .venv exists or can be created
    if (project_path / ".venv").exists():
        ...
        checks.append({
            "check": ".venv/ exists",
            "passed": False,
            "detail": "No virtual environment found (run 'uv sync')",
        })
...
def _check_rust(project_path: Path) -> list[dict]:
    """Health checks for Rust projects."""
    checks = [
...
def _check_flutter(project_path: Path) -> list[dict]:
    """Health checks for Flutter projects."""
    checks = [
    ...
    Returns:
        {
            ...
            "checks": [
                {"check": "package.json exists", "passed": True, "detail": "..."},
                ...
            ],
            "passed": 5,
            "failed": 1,
