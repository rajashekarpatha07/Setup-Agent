...
Usage:
    ...
    dash = Dashboard()
    dash.start()                          # start live rendering
    dash.set_task("flask api", "Setup a flask api")
    dash.add_step("npm init -y", "success")
    dash.set_idle()                       # back to listening
    dash.stop()                           # stop rendering
"""
...
class Dashboard:
    ...
    def __init__(self, console: Console | None = None):
        ...
        self._steps: list[dict] = []        # {"command": str, "status": str, "duration": float}
    ...
    def add_step(self, command: str, status: str = "running", duration: float = 0.0):
        """Add or update a step in the current task."""
        with self._lock:
    ...
    def _render_header(self) -> Panel:
        ...
        emoji, status_text, color = STATUS_DISPLAY.get(
            self._status, STATUS_DISPLAY["listening"]
        )
        ...
        return Panel(
            table,
            title="[bold bright_white]🤖 Setup Agent[/]",
    ...
    def _render_task(self) -> Panel:
        ...
        # Step list
        for step in self._steps[-10:]:  # show last 10 steps
            cmd = step["command"]
            if len(cmd) > 60:
                cmd = cmd[:57] + "..."
        ...
        return Panel(
            content,
            title=f"[bold]Current Task — [cyan]{self._current_task}[/][/]",
            border_style="bright_blue",
            box=box.ROUNDED,
        )

    def _render_history(self) -> Panel:
        """Render the recent history panel."""
        try:
        ...
        if not recent:
            return Panel(
                Align.center(Text("No projects yet", style="dim italic"), vertical="middle"),
                title="[bold]Recent History[/]",
                border_style="dim",
                box=box.ROUNDED,
            )
